...
import { loadConnectionConfig } from "../lib/config.js";
...
server.tool(
  ...
  async ({ query, limit }) => {
    try {
      return textResult(await graph.searchMemory(query, limit ?? 10));
    } catch (error) {
...
server.tool(
  ...
  async ({ name }) => {
    try {
      const entity = await graph.getEntity(name);
...
server.tool(
  ...
  "Record one or more durable facts/observations about an entity (a person, project, decision, preference, or convention). Creates the entity if it doesn't exist yet. Only record things worth remembering across sessions, not ephemeral chit-chat.",
  {
  ...
  async ({ entity, entityType, observations }) => {
    try {
      ...
      return textResult({ entity, added: ids.length, observationIds: ids });
    } catch (error) {
      return errorResult(error);
    }
  }
...
server.tool(
  ...
  "Create a directed, typed relationship between two entities in the memory graph, e.g. from='claude-neo4j' type='uses' to='neo4j-driver'. Creates either entity if missing.",
...
server.tool(
  ...
  "Delete specific observations from an entity by observation id (ids are returned from memory_add_observations or memory_get_entity).",
  { entity: z.string(), observationIds: z.array(z.string()).min(1) },
  async ({ entity, observationIds }) => {
    try {
      const deleted = await graph.deleteObservations(entity, observationIds);
...
server.tool(
  ...
  "Delete an entity and all of its observations and relations. Use when the user asks you to forget something.",
  { entity: z.string() },
  async ({ entity }) => {
    try {
      await graph.deleteEntity(entity);
      return textResult({ ok: true });
    } catch (error) {
      return errorResult(error);
    }
